...
package com.opendatagroup.fastscore.swagger.invoker1
...
class ApiInvoker(val mapper: ObjectMapper = ScalaJsonUtil.getJsonMapper,
  ...
  def escape(value: UUID): String = value.toString;	def escape(value: Integer): String = value.toString
  ...
  def invokeApi(host: String, path: String, method: String, queryParams: Map[String, String], formParams: Map[String, String], body: AnyRef, headerParams: Map[String, String], contentType: String, accept: String = "application/json"): String = {
    ...
    val querystring = if (queryParams.isEmpty) "" else queryParams.filter(k => k._2 != null).map(k => (escape(k._1) + "=" + escape(k._2))).mkString("?", "&", "")
    val builder = client.resource(host + path + querystring).accept(accept)
    ...
    val response: ClientResponse = method match {
      ...
      case "POST" => {
        if(formData != null) builder.post(classOf[ClientResponse], formData)
          ...
          if(body == null) builder.post(classOf[ClientResponse], body)
...
object ApiInvoker extends ApiInvoker(mapper = ScalaJsonUtil.getJsonMapper,
  httpHeaders = HashMap(("Accept" -> "application/json")),
  hostMap = HashMap(),
