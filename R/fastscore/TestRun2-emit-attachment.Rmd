---
title: "Fastscore R-SDK Example 2"
...
engine$load_model(model = mm$model_get("echo-array-double.R"))  # load model into engine
...
engine$load_model(mm$model_get("echo-array-double.R"))  # load model into engine
engine$input_set(0, mm$stream_get("array-double-input"))  # set input stream, use even slot numbers 0, 2, 4, etc.
engine$output_set(1, mm$stream_get("array-double-output"))  # set output stream, use odd slot numbers 1, 3, 5, etc.
```
Stop engine
```{r}
engine$unload_model()  # reset engine for next deployment
