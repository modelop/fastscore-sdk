# This is a slightly-modified version of Titus's datatype library
# We have moved it here for compatibility with Python3 (which Titus does not support)
...
def jsonToAvroType(x):
    """Convert a JSON-encoded Avro type (AVSC file) into a fastscore.datatype.AvroType.

    :type x: string
    :param x: JSON-encoded Avro type
    :rtype: fastscore.datatype.AvroType
...
def jsonNodeToAvroType(x):
    """Convert a Python-encoded Avro type into a fastscore.datatype.AvroType.
...
def schemaToAvroType(schema):
    """Convert an Avro schema into a fastscore.datatype.AvroType.
...
class AvroType(Type):
    ...
    def accepts(self, other, memo=None, checkRecord=True):
        ...
        :type self: fastscore.datatype.AvroType
        :param self: the expected signature to be matched
        :type other: fastscore.datatype.AvroType
...
class AvroArray(AvroContainer):
    ...
    def __init__(self, items):
        ...
        :type items: fastscore.datatype.AvroType
...
class AvroMap(AvroContainer, AvroMapping):
    ...
    def __init__(self, values):
        ...
        :type values: fastscore.datatype.AvroType
...
class AvroRecord(AvroContainer, AvroMapping, AvroCompiled):
    ...
    def __init__(self, fields, name=None, namespace=None):
        ...
        :type fields: list of fastscore.datatype.AvroField
    ...
    @property
    def fields(self):
        """Get the fields as a list of fastscore.datatype.AvroField objects."""
        return [AvroField.fromSchema(x) for x in self.schema.fields]
    @property
    def fieldsDict(self):
        """Get the fields as a dict from field names to fastscore.datatype.AvroField objects."""
        return dict((x.name, x) for x in self.fields)
    def field(self, name):
        """Get one fastscore.datatype.AvroField object by name."""
...
class AvroUnion(AvroType):
    ...
    def __init__(self, types):
        ...
        :type types: list of fastscore.datatype.AvroType
        :param types: possible types for this union in the order of their resolution
        """
        names = set([x.name for x in types])
        if len(types) != len(names):
            raise errors.AvroException("duplicate in union: " + ", ".join(map(ts, types)))
        if "union" in names:
            raise errors.AvroException("nested union: " + ", ".join(map(ts, types)))
        self._schema = avro.schema.UnionSchema([], avro.schema.Names())
        self._schema._schemas = [x.schema for x in types]
    @property
    def types(self):
        """Get the possible types for this union as a list of fastscore.datatype.AvroType."""
...
class AvroField(object):
    """Field of a fastscore.datatype.AvroRecord."""
    @staticmethod
    ...
    @property
    def avroType(self):
        """Get the field type as a fastscore.datatype.AvroType."""
    ...
    def default(self):
        ...
        except KeyError: # Python2 Avro returns None, Python3 returns KeyError
            return None
    @property
    def order(self):
        """Get the field order."""
...
class AvroPlaceholder(object):
    ...
    @property
    def avroType(self):
        """Called after ``fastscore.datatype.AvroTypeBuilder`` ``resolveTypes`` to get the resolved type."""
    ...
    @property
    def parser(self):
        """The ``fastscore.datatype.ForwardDeclarationParser`` responsible for this placeholder."""
        return self.forwardDeclarationParser

class AvroFilledPlaceholder(AvroPlaceholder):
    """Used to create fastscore.datatype.AvroPlaceholder objects to satisfy functions that require them, yet the type is already known."""
...
def parseAvroType(obj):
    ...
    if six.PY3:
        return schemaToAvroType(avro.schema.SchemaFromJSONData(json_data=obj, names=avro.schema.Names()))
    else:
        return schemaToAvroType(avro.schema.make_avsc_object(obj, avro.schema.Names()))

class ForwardDeclarationParser(object):
    """Container that stores Avro types as they're collected from a PFA file, returning fastscore.datatype.AvroPlaceholder objects, and then resolves those types independent of the order in which they were read from the file."""
    ...
    def parse(self, jsonStrings):
        ...
        while unresolvedSize != 0:
                ...
                if jsonString not in schemae:
                    ...
                    if isinstance(obj, str) and self.names.has_name(obj, None):
                        ...
                        try:
                            gotit = None
                            if six.PY3:
                                gotit = avro.schema.SchemaFromJSONData(json_data=obj, names=self.names)
                            else:
            ...
            if unresolvedSize == lastUnresolvedSize:
                raise errors.SchemaParseException("Could not resolve the following types:\n    " +
        ...
        if isinstance(description, str):
            ...
            if description == {"type": "null"}:
                ...
                if self.names.has_name(description.get("name"), description.get("namespace")):
                    ...
                    raise errors.AvroException("new types, like {0}, cannot be defined with the parser.getAvroType or parser.getSchema methods (use parse)".format(json.dumps(description)))
...
class AvroTypeBuilder(object):
    """Factory that coordinates the process of collecting Avro strings, putting them in a fastscore.datatype.ForwardDeclarationParser, and then resolving them all at the end, independent of order."""
...
def compare(avroType, x, y):
    ...
    if isinstance(avroType, AvroNull) and x is None and y is None:
        ...
        raise errors.AvroException("{0} or {1} does not match schema {2}".format(json.dumps(x), json.dumps(y), ts(avroType)))
